...
public class Pert {
    ...
    private List<Activity> lastActivity;
    private List<Activity> activities;
    private double criticalPathValue, sigmaSquared, X, P;  // deadline = czas dyrektywy, criticalPathValue = czas modelowy
    private String criticalPath, path;
    private String []ids;
    private List<String> paths;
    private int id, deadline;
    
    public Pert(int deadline){
        this.deadline = deadline;
        lastActivity = new ArrayList<>();
    ...
    public void doProbability(){     // startpoint
        findLastAcitivites();
        doCriticPath();
        sumSigma();
        calculateX();
        getFromNormalList();
        GUI.getLabel1().setText(GUI.getLabel1().getText() + criticalPath);
        GUI.getLabel2().setText(GUI.getLabel2().getText() + criticalPathValue);
        GUI.getLabel3().setText(GUI.getLabel3().getText() + sigmaSquared);
        GUI.getLabel4().setText(GUI.getLabel4().getText() + "<" + (criticalPathValue-sigmaSquared) + ", " + (criticalPathValue+sigmaSquared) + ">");
        GUI.getLabel5().setText(GUI.getLabel5().getText() + X);
        GUI.getLabel6().setText(GUI.getLabel6().getText() + P + "%");
    }
    
    public void doCriticPath(){
        criticalPathValue = 0;
        if(!lastActivity.isEmpty()){
            for(Activity a: lastActivity){
                sumValues(a, 0);
                path = "";            
            }
        }
    ...
    public void sumValues(Activity a, double sum){
        double localSum = sum + a.getT0();
        path += Integer.toString(a.getId())+",";
        String tmp1 = path;
        if(!a.getPreviousList().isEmpty()){
            for(Activity act: a.getPreviousList()){
                path = tmp1;
                sumValues(act, localSum);
            }
        }
        else{
            if(localSum>criticalPathValue){
                criticalPathValue = localSum;
                criticalPath = path;
            }
        }
    }
    
    public void findSigma(Activity act){
        if(!act.getPreviousList().isEmpty()){
            String s = "" + criticalPath.charAt(id);
            for (Activity a: act.getPreviousList()) {
                if(a.getId() == Integer.parseInt(ids[id])){
                    sigmaSquared += a.getSigma();
                    id++;
                    findSigma(a);
                    break;
                }
    ...
    public void sumSigma(){
       String s = "" + criticalPath.charAt(id);
       ids = criticalPath.split(",");
        for (Activity a: lastActivity) {
            if(a.getId() == Integer.parseInt(ids[id])){
                sigmaSquared += a.getSigma();
                id++;
        ...
        sigmaSquared = Math.round(sigmaSquared*100);
        sigmaSquared /= 100;
    }
    
    public void findLastAcitivites(){
        boolean stop;
        for(Activity a: activities){
            stop = false;
            for(Activity b: activities){
                for(Activity c: b.getPreviousList()){
    ...
    public void getFromNormalList(){
        if(deadline < (criticalPathValue-sigmaSquared)){
           P = 0;
        }
        else if(deadline > (criticalPathValue+sigmaSquared)){
            P = 100;
        }
        else{
            int index;
            if(X>=0){
                index = (int) (X*100);
                P = normalDistribution[index]*10000;
            }
            else{
                index = (int) (-X*100);
                P = 10000 - (normalDistribution[index]*10000);
            }
            P = Math.round(P);
            P /= 100;
        }
    ...
    public void addPreviousActivities(){
        for(Activity a: activities){
            String[] tab = a.getPreviousActivities().split(";");
            for (int i = 0; i < tab.length; i++) {
                for(Activity act: activities){
                    if(act.getId() == Integer.parseInt(tab[i])){
                        a.addPrevious(act);
                    }
                }
            }
    ...
    public void calculateX(){
        X = ((deadline-criticalPathValue)/Math.sqrt(sigmaSquared))*100;
        X = Math.round(X);
        X /= 100;
    }
    
    public List<Activity> getLastActivity() {
        return lastActivity;
    }

    public double getCriticalPathValue() {
        return criticalPathValue;
    }

    public void setCriticalPathValue(double criticalPathValue) {
        this.criticalPathValue = criticalPathValue;
    }
    ...
    private double[] normalDistribution = new double[] {
